...
def build_model_input(form_data: dict, model_features: list) -> dict:
    ...
    exchange_count = map_count(form_data.get("exchange_count", "없음")) if accident_flag else 0
    paint_count = map_count(form_data.get("paint_count", "없음")) if accident_flag else 0
    insurance_count = map_count(form_data.get("insurance_count", "없음")) if accident_flag else 0
    ...
    if "차급_라벨" in row:
        row["차급_라벨"] = map_vehicle_class_label(form_data.get("vehicle_class", "중형"))
